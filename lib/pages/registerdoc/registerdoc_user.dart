...
class DropDownState extends State<RegisterDocUser> {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton(
          ...
          onPressed: () async {
            ...
            //String username = args == null ? "" : args.username;
  ...
  void thongbao(username) async {
    var intro = Intro.create(1, 1, "PO", "Tiêu đề giới hiệu", "Mô tả công ty",
        "2019/1/1", "2019/2/2");
    ...
    showDialog(
        ...
        builder: (_) => NetworkGiffyDialog(
            ...
            title: Text('Thêm mới Giới thiệu',
                textAlign: TextAlign.center,
                style: TextStyle(
                    fontSize: 22.0,
                    fontWeight: FontWeight.w600,
                    color: Colors.lightBlue)),
            description: Text(username + '. Thêm mới Giới thiệu thành công.',
                textAlign: TextAlign.center),
            onOkButtonPressed: () {
  ...
  signIn(String email, pass, platformImei) async {
    ...
    Response response = await dio.post('/api/appuser/login',
        data: {"UserName": email, "Password": pass}, options: options);

    if (response.statusCode == 200) {
      _apiResponse.Data = Login.fromJson(json.decode(response.data));

      SharedPreferences prefs = await SharedPreferences.getInstance();
      await prefs.setString("token", (_apiResponse.Data as Login).token);
      await prefs.setString(
          "useremail", (_apiResponse.Data as Login).useremail);
      await prefs.setString("userid", (_apiResponse.Data as Login).userid);
      await prefs.setString("username", (_apiResponse.Data as Login).username);
      await prefs.setString("avatar", (_apiResponse.Data as Login).avatar);
      await prefs.setString("fullname", (_apiResponse.Data as Login).fullname);

      var blocHome2Provider = BlocProvider(bloc: HomeBloc(), child: HomePage());
      await Navigator.push(context,
          MaterialPageRoute<bool>(builder: (context) => blocHome2Provider));
  ...
  addRegisterDoc(firebasenotifiId, username, softid, softname, platformImei,
      culture) async {
    Options options = Options(
      contentType: "application/json",
    );

    Response response = await dio.post('/api/registerdoc?firebasenotifiId=' +
        firebasenotifiId +
        '&username=' +
        username +
        '&softId=' +
...
Future<List<RegisterDoc>> fetchRegisterDoc(http.Client client) async {
  final response =
      await client.get('http://powaco.ddns.net:85/api/vi-VN/categorynews/1');
